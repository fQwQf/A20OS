...
ifeq ($(RUST_ENABLED),1)
  ...
  ifeq ($(RUST_MODULE_WAIT),1)
    CFLAGS += -DCONFIG_RUST_WAIT
    RUST_LIBS += $(BUILD_DIR)/rust/libwait.rlib
  endif
  ifeq ($(RUST_MODULE_PROC_CORE),1)
    CFLAGS += -DCONFIG_RUST_PROC_CORE
    RUST_LIBS += $(BUILD_DIR)/rust/libproc_core.rlib
  endif
  ifeq ($(RUST_MODULE_SOCKET_REGISTRY),1)
    CFLAGS += -DCONFIG_RUST_SOCKET_REGISTRY
    RUST_LIBS += $(BUILD_DIR)/rust/libsocket_registry.rlib
  ...
  ifeq ($(RUST_MODULE_SOCKET_UNIX),1)
    CFLAGS += -DCONFIG_RUST_SOCKET_UNIX
    RUST_LIBS += $(BUILD_DIR)/rust/libsocket_unix.rlib
    RUST_SUPPORT_SRC += kernel/rust/support/socket_unix_helpers.c
    RUST_SUPPORT_COBJ += $(BUILD_DIR)/rust/socket_unix_helpers.o
  endif
  ifeq ($(RUST_MODULE_SOCKET_FILE),1)
    CFLAGS += -DCONFIG_RUST_SOCKET_FILE
    RUST_LIBS += $(BUILD_DIR)/rust/libsocket_file.rlib
    RUST_SUPPORT_SRC += kernel/rust/support/socket_file_helpers.c
    RUST_SUPPORT_COBJ += $(BUILD_DIR)/rust/socket_file_helpers.o
  endif
  ifeq ($(RUST_MODULE_SOCKET_INET_ADDR),1)
    CFLAGS += -DCONFIG_RUST_SOCKET_INET_ADDR
    RUST_LIBS += $(BUILD_DIR)/rust/libsocket_inet_addr.rlib
    RUST_SUPPORT_SRC += kernel/rust/support/socket_inet_addr_helpers.c
    RUST_SUPPORT_COBJ += $(BUILD_DIR)/rust/socket_inet_addr_helpers.o
  endif
  ifeq ($(RUST_MODULE_SOCKET_INET_BH),1)
    CFLAGS += -DCONFIG_RUST_SOCKET_INET_BH
    RUST_LIBS += $(BUILD_DIR)/rust/libsocket_inet_bh.rlib
  ...
  ifeq ($(RUST_MODULE_SOCKET_CORE),1)
    CFLAGS += -DCONFIG_RUST_SOCKET_CORE
    RUST_LIBS += $(BUILD_DIR)/rust/libsocket_core.rlib
...
ifeq ($(RUST_ENABLED),1)
  ...
  ifeq ($(RUST_MODULE_WAIT),1)
    RUST_KERNEL_CFG += --cfg rust_module_wait
  endif
  ifeq ($(RUST_MODULE_PROC_CORE),1)
    RUST_KERNEL_CFG += --cfg rust_module_proc_core
  endif
  ifeq ($(RUST_MODULE_SOCKET_REGISTRY),1)
  ...
  ifeq ($(RUST_MODULE_SOCKET_UNIX),1)
    RUST_KERNEL_CFG += --cfg rust_module_socket_unix
  endif
  ifeq ($(RUST_MODULE_SOCKET_FILE),1)
    RUST_KERNEL_CFG += --cfg rust_module_socket_file
  endif
  ifeq ($(RUST_MODULE_SOCKET_INET_ADDR),1)
    RUST_KERNEL_CFG += --cfg rust_module_socket_inet_addr
  endif
  ifeq ($(RUST_MODULE_SOCKET_INET_BH),1)
  ...
  ifeq ($(RUST_MODULE_SOCKET_CORE),1)
    RUST_KERNEL_CFG += --cfg rust_module_socket_core
  endif
  ifeq ($(RUST_MODULE_XATTR),1)
...
ifeq ($(RUST_MODULE_WAIT),1)
KERNEL_SRC := $(filter-out $(KERNEL_DIR)/proc/wait.c,$(KERNEL_SRC))
endif
ifeq ($(RUST_MODULE_PROC_CORE),1)
KERNEL_SRC := $(filter-out $(KERNEL_DIR)/proc/proc.c,$(KERNEL_SRC))
endif
...
ifeq ($(RUST_MODULE_SOCKET_UNIX),1)
KERNEL_SRC := $(filter-out $(KERNEL_DIR)/net/socket_unix.c,$(KERNEL_SRC))
endif
ifeq ($(RUST_MODULE_SOCKET_FILE),1)
KERNEL_SRC := $(filter-out $(KERNEL_DIR)/net/socket_file.c,$(KERNEL_SRC))
endif
ifeq ($(RUST_MODULE_SOCKET_CORE),1)
...
$(BUILD_DIR)/rust/futex_helpers.o: kernel/rust/support/futex_helpers.c Makefile
	@mkdir -p $(dir $@)
	$(CROSS_PREFIX)gcc $(CFLAGS) -c $< -o $@
...
$(BUILD_DIR)/rust/wait_helpers.o: kernel/rust/support/wait_helpers.c Makefile
	@mkdir -p $(dir $@)
	$(CROSS_PREFIX)gcc $(CFLAGS) -c $< -o $@

$(BUILD_DIR)/rust/proc_core_helpers.o: kernel/rust/support/proc_core_helpers.c Makefile
	@mkdir -p $(dir $@)
	$(CROSS_PREFIX)gcc $(CFLAGS) -c $< -o $@
...
$(BUILD_DIR)/rust/socket_unix_helpers.o: kernel/rust/support/socket_unix_helpers.c Makefile
	@mkdir -p $(dir $@)
	$(CROSS_PREFIX)gcc $(CFLAGS) -c $< -o $@

$(BUILD_DIR)/rust/socket_file_helpers.o: kernel/rust/support/socket_file_helpers.c Makefile
	@mkdir -p $(dir $@)
	$(CROSS_PREFIX)gcc $(CFLAGS) -c $< -o $@

$(BUILD_DIR)/rust/socket_inet_addr_helpers.o: kernel/rust/support/socket_inet_addr_helpers.c Makefile
	@mkdir -p $(dir $@)
	$(CROSS_PREFIX)gcc $(CFLAGS) -c $< -o $@

$(BUILD_DIR)/rust/socket_inet_bh_helpers.o: kernel/rust/support/socket_inet_bh_helpers.c Makefile
	@mkdir -p $(dir $@)
	$(CROSS_PREFIX)gcc $(CFLAGS) -c $< -o $@
...
$(BUILD_DIR)/rust/socket_core_helpers.o: kernel/rust/support/socket_core_helpers.c Makefile
	@mkdir -p $(dir $@)
	$(CROSS_PREFIX)gcc $(CFLAGS) -c $< -o $@
...
$(BUILD_DIR)/rust/libproc_core.rlib: kernel/rust/proc_core/lib.rs kernel/rust/proc_core/ffi.rs $(RUST_SUPPORT_LIB) Makefile
	@mkdir -p $(dir $@)
	$(RUSTC) $(RUSTFLAGS) --crate-name proc_core --extern a20rust_support=$(RUST_SUPPORT_LIB) $< -o $@
...
$(BUILD_DIR)/rust/libsocket_queue.rlib: kernel/rust/socket_queue/lib.rs kernel/rust/socket_queue/ffi.rs $(RUST_SUPPORT_LIB) Makefile
	@mkdir -p $(dir $@)
	$(RUSTC) $(RUSTFLAGS) $(if $(filter 1,$(RUST_MODULE_SOCKET_CORE)),--cfg rust_module_socket_core) --crate-name socket_queue --extern a20rust_support=$(RUST_SUPPORT_LIB) $< -o $@

$(BUILD_DIR)/rust/libsocket_unix.rlib: kernel/rust/socket_unix/lib.rs kernel/rust/socket_unix/ffi.rs $(RUST_SUPPORT_LIB) Makefile
	@mkdir -p $(dir $@)
	$(RUSTC) $(RUSTFLAGS) --crate-name socket_unix --extern a20rust_support=$(RUST_SUPPORT_LIB) $< -o $@

$(BUILD_DIR)/rust/libsocket_file.rlib: kernel/rust/socket_file/lib.rs kernel/rust/socket_file/ffi.rs $(RUST_SUPPORT_LIB) Makefile
	@mkdir -p $(dir $@)
	$(RUSTC) $(RUSTFLAGS) --crate-name socket_file --extern a20rust_support=$(RUST_SUPPORT_LIB) $< -o $@

$(BUILD_DIR)/rust/libsocket_inet_addr.rlib: kernel/rust/socket_inet_addr/lib.rs kernel/rust/socket_inet_addr/ffi.rs $(RUST_SUPPORT_LIB) Makefile
	@mkdir -p $(dir $@)
	$(RUSTC) $(RUSTFLAGS) --crate-name socket_inet_addr --extern a20rust_support=$(RUST_SUPPORT_LIB) $< -o $@
...
$(BUILD_DIR)/rust/libsocket_core.rlib: kernel/rust/socket_core/lib.rs kernel/rust/socket_core/ffi.rs $(RUST_SUPPORT_LIB) Makefile
	@mkdir -p $(dir $@)
	$(RUSTC) $(RUSTFLAGS) --crate-name socket_core --extern a20rust_support=$(RUST_SUPPORT_LIB) $< -o $@
