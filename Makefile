...
benchmark-rv:
	...
	$(MAKE) ARCH=riscv64 benchmark=1 _reset_obj
...
benchmark-la:
	...
	$(MAKE) ARCH=loongarch64 benchmark=1 _reset_obj
...
_benchmark_build: $(KERNEL_ELF) user_apps _benchmark_disk
	cp $(KERNEL_ELF) $(KERNEL_OUT)
	@echo "  -> $(KERNEL_OUT) + $(DISK_OUT)"

_benchmark_disk: user_apps
...
user_apps:
	...
	@if [ ! -f "$(USER_BUILD_STAMP)" ] || [ "$$(cat "$(USER_BUILD_STAMP)")" != "$(ARCH):$(benchmark)" ]; then \
		$(MAKE) -C user clean; \
	fi
ifeq ($(benchmark),1)
	$(MAKE) -C user ARCH=$(ARCH) benchmark=$(benchmark) build_dir build/init
else
	$(MAKE) -C user ARCH=$(ARCH) benchmark=$(benchmark)
endif
	@printf '%s\n' '$(ARCH):$(benchmark)' > $(USER_BUILD_STAMP)
