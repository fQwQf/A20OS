...
.PHONY: all clean run-riscv64 run-loongarch64 run-arm64 run-x86_64 run-arm32 run-riscv32 run-ppc64le debug-riscv64 debug-loongarch64 debug-arm64 debug-x86_64 debug-arm32 debug-riscv32 debug-ppc64le \
		check-kernel-build check-user-build check-dev-build check-benchmark-build check-build-matrix check-abi-smoke-gate check-doc-drift check-doc-test-gates check-final-definition check-concurrency-foundation check-mm-lock-model check-abi-boundary check-driver-core-model check-external-dependency-boundary \
...
_benchmark_build: $(KERNEL_ELF) $(USER_BUILD_STAMP)
ifeq ($(ARCH), riscv64)
	$(MAKE) native-test-rv
