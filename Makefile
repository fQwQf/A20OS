...
.PHONY: all all-architectures clean run-riscv64 run-gui-riscv64 run-gui-rv run-loongarch64 run-gui-loongarch64 run-gui-la run-arm64 run-gui-arm64 run-gui-aarch64 run-x86_64 run-gui-x86_64 vbox-iso-x86_64 _vbox_iso_x86_64_impl vbox-image-aarch64 _vbox_image_aarch64_impl vbox-text-image-aarch64 _vbox_text_image_aarch64_impl run-arm32 run-gui-arm32 run-riscv32 run-ppc64le debug-riscv64 debug-loongarch64 debug-arm64 debug-x86_64 debug-arm32 debug-riscv32 debug-ppc64le \
		...
		check-stm32f103 \
		check-kernel-build check-kernel-build-all check-user-build check-user-build-all check-dev-build check-benchmark-build check-benchmark-build-all check-build-matrix check-build-matrix-all check-abi-smoke-gate check-doc-drift check-doc-test-gates check-final-definition check-concurrency-foundation check-mm-lock-model check-abi-boundary check-driver-core-model check-external-dependency-boundary \
		...
		user_apps fs_img kernel-only dev-build benchmark-rv benchmark-la \
...
# --- eval dev-build targets (match run-*, add benchmark-mode + 128 MB) ---
...
eval-dev-build-rv:
	...
	@printf 'auto\n' | mcopy -o -i $(EVAL_FAT32_RV) - ::/etc/benchmark-mode

eval-dev-build-la:
	$(MAKE) ARCH=loongarch64 FAT32_IMAGE_MB=128 dev-build
	@printf 'auto\n' | mcopy -o -i $(EVAL_FAT32_LA) - ::/etc/benchmark-mode
...
eval-check-rv:
	...
	grep -q '\[benchmark\] Done:' "$$log" || { echo "[eval][rv] incomplete: missing [benchmark] Done"; exit 1; }; \
...
eval-check-la:
	...
	grep -q '\[benchmark\] Done:' "$$log" || { echo "[eval][la] incomplete: missing [benchmark] Done"; exit 1; }; \
...
# ----------------------------------------------------------------
# 2026 release evaluation.  Each target builds an 8-CPU image, restores a
...
# official judge, and archives the complete run under .kernel-build/smoke/2026.
# ----------------------------------------------------------------
...
release-eval-rv-functional tests:
	$(call RUN_RELEASE_EVAL,riscv64,functional tests)

release-eval-la-functional tests:
	$(call RUN_RELEASE_EVAL,loongarch64,functional tests)

release-eval-rv-parallel-build:
	$(call RUN_RELEASE_EVAL,riscv64,parallel-build)
...
final-stage7-rv-shebang:
	$(call RUN_RELEASE_EVAL,riscv64,parallel-build-probe,1,stage7-shebang-exec,900)
...
final-stage7-la-1c-j1:
	$(call RUN_RELEASE_EVAL,loongarch64,parallel-build-probe,1,stage7-full-j1,28800)

final-stage7-rv-8c-j1:
	$(call RUN_RELEASE_EVAL,riscv64,parallel-build-probe,8,stage7-full-j1,28800)

final-stage7-la-8c-j1:
	$(call RUN_RELEASE_EVAL,loongarch64,parallel-build-probe,8,stage7-full-j1,28800)

final-stage7-rv-8c-j2:
	$(call RUN_RELEASE_EVAL,riscv64,parallel-build-probe,8,stage7-full-j2,28800)

final-stage7-la-8c-j2:
	$(call RUN_RELEASE_EVAL,loongarch64,parallel-build-probe,8,stage7-full-j2,28800)

final-stage7-rv-8c-j4:
	$(call RUN_RELEASE_EVAL,riscv64,parallel-build-probe,8,stage7-full-j4,28800)

final-stage7-la-8c-j4:
	$(call RUN_RELEASE_EVAL,loongarch64,parallel-build-probe,8,stage7-full-j4,28800)

final-stage7-rv-8c-j8:
	$(call RUN_RELEASE_EVAL,riscv64,parallel-build-probe,8,stage7-full-j8,28800)

final-stage7-la-8c-j8:
	$(call RUN_RELEASE_EVAL,loongarch64,parallel-build-probe,8,stage7-full-j8,28800)

final-stage7-rv-8c-default:
	$(call RUN_RELEASE_EVAL,riscv64,parallel-build-probe,8,stage7-full-default,3000)

final-stage7-la-8c-default:
	$(call RUN_RELEASE_EVAL,loongarch64,parallel-build-probe,8,stage7-full-default,3000)
