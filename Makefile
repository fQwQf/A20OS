...
BUILD_VARIANT = $(ABI)-$(if $(filter 1,$(benchmark)),benchmark,$(if $(filter 1,$(BRINGUP)),bringup,dev))
...
FAT32_IMAGE_MB ?= 32
EXT4_IMAGE_MB ?= 32
benchmark_DISK_MB ?= $(FAT32_IMAGE_MB)
EXTRA_IMAGE_MB ?= 256
EXTRA_IMG = $(BUILD_DIR)/extra.img
EXTRA_STAGING_DIR = $(BUILD_DIR)/extra-staging
EXTRA_PACKAGES = vim git gcc cc
USER_BUILD_ID = $(ARCH):$(benchmark):$(OPT)
USER_BUILD_CHECK_DIRS = user/cmds user/benchmark_init user/init_common user/lib user/shell \
...
.PHONY: all clean run-riscv64 run-loongarch64 run-arm64 debug-riscv64 debug-loongarch64 debug-arm64 \
        check-kernel-build check-user-build check-dev-build check-benchmark-build \
...
# ----------------------------------------------------------------
# reference build: produces kernel-rv, kernel-la, disk.img,
...
check-riscv64-user:
	$(MAKE) -C user ARCH=riscv64 benchmark=$(benchmark) OPT="$(OPT)"

check-loongarch64-user:
	$(MAKE) -C user ARCH=loongarch64 benchmark=$(benchmark) OPT="$(OPT)"

check-aarch64-user:
	$(MAKE) -C user ARCH=aarch64 benchmark=$(benchmark) OPT="$(OPT)"

check-dev-build:
	$(MAKE) ARCH=riscv64 ABI=$(ABI) BRINGUP=0 benchmark=0 dev-build
...
smoke-abi-linux:
	$(MAKE) ARCH=riscv64 ABI=linux BRINGUP=0 benchmark=0 dev-build
	@mkdir -p $(SMOKE_LOG_DIR)
	@set -e; \
...
benchmark-rv:
	...
	$(MAKE) ARCH=riscv64 benchmark=1 _benchmark_build KERNEL_OUT=kernel-rv DISK_OUT=disk.img

benchmark-la:
	@echo "--- Building LoongArch 64 (benchmark) ---"
	$(MAKE) ARCH=loongarch64 benchmark=1 _benchmark_build KERNEL_OUT=kernel-la DISK_OUT=disk-la.img
...
_benchmark_build: $(KERNEL_ELF) $(USER_BUILD_STAMP) _benchmark_disk
	cp $(KERNEL_ELF) $(KERNEL_OUT)
	@echo "  -> $(KERNEL_OUT) + $(DISK_OUT)"

_benchmark_disk: $(USER_BUILD_STAMP)
	dd if=/dev/zero of=$(DISK_OUT) bs=1M count=$(benchmark_DISK_MB) 2>/dev/null
...
$(USER_BUILD_STAMP): FORCE
	...
	if [ "$$need_build" -eq 1 ]; then \
		...
		$(MAKE) -C user ARCH=$(ARCH) benchmark=$(benchmark) OPT="$(OPT)"; \
		printf '%s\n' '$(USER_BUILD_ID)' > "$@"; \
	else \
...
run-riscv64:
	$(MAKE) ARCH=riscv64 BRINGUP=$(BRINGUP) benchmark=$(benchmark) _run_impl

run-loongarch64:
	$(MAKE) ARCH=loongarch64 BRINGUP=$(BRINGUP) benchmark=$(benchmark) _run_impl

run-arm64:
	$(MAKE) ARCH=aarch64 BRINGUP=$(BRINGUP) benchmark=$(benchmark) _run_impl

_run_impl:
ifeq ($(BRINGUP),1)
	$(MAKE) ARCH=$(ARCH) BRINGUP=1 benchmark=$(benchmark) kernel-only
else
	$(MAKE) ARCH=$(ARCH) BRINGUP=$(BRINGUP) benchmark=$(benchmark) dev-build
endif
...
debug-riscv64:
	$(MAKE) ARCH=riscv64 BRINGUP=$(BRINGUP) benchmark=$(benchmark) _debug_impl

debug-loongarch64:
	$(MAKE) ARCH=loongarch64 BRINGUP=$(BRINGUP) benchmark=$(benchmark) _debug_impl

debug-arm64:
	$(MAKE) ARCH=aarch64 BRINGUP=$(BRINGUP) benchmark=$(benchmark) _debug_impl

_debug_impl:
ifeq ($(BRINGUP),1)
	$(MAKE) ARCH=$(ARCH) BRINGUP=1 benchmark=$(benchmark) CFLAGS="$(DEBUG_CFLAGS)" kernel-only
else
	$(MAKE) ARCH=$(ARCH) BRINGUP=$(BRINGUP) benchmark=$(benchmark) CFLAGS="$(DEBUG_CFLAGS)" kernel-only
	$(MAKE) -C user ARCH=$(ARCH) benchmark=$(benchmark) OPT="-O0 -g"
endif
...
run-riscv64-extra:
	$(MAKE) ARCH=riscv64 BRINGUP=0 benchmark=0 _run_extra_impl

run-loongarch64-extra:
	$(MAKE) ARCH=loongarch64 BRINGUP=0 benchmark=0 _run_extra_impl

run-arm64-extra:
	$(MAKE) ARCH=aarch64 BRINGUP=0 benchmark=0 _run_extra_impl

_run_extra_impl:
	$(MAKE) ARCH=$(ARCH) BRINGUP=0 benchmark=0 dev-build
