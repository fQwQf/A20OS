...
static void append_arch_ltp_dirs(char *buf, size_t buf_size)
{
#if defined(__loongarch64)
    append_existing_dir(buf, buf_size, "/testla/musl/ltp/testcases/bin");
    append_existing_dir(buf, buf_size, "/testrv/musl/ltp/testcases/bin");
#else
    append_existing_dir(buf, buf_size, "/testrv/musl/ltp/testcases/bin");
    append_existing_dir(buf, buf_size, "/testla/musl/ltp/testcases/bin");
#endif
...
static int build_test_exec_path(const char *runtime, char *buf, size_t buf_size)
{
    ...
    append_arch_ltp_dirs(buf, buf_size);
    return 0;
}
