...
/* ============================================================
 * benchmark mode — scan-only helper
 *
 * Recursively scans /test (reference EXT4 disk), prints a
...
int main(int argc, char *argv[])
{
    ...
    printf("[benchmark] Scan-only init started\n");
    printf("[benchmark] Scanning target image at %s\n", TEST_DIR);
    ...
    printf("[benchmark] Found %d test script(s) matching %s\n", ntests, SUFFIX);
    for (int i = 0; i < ntests; i++) {
        printf("[benchmark] TEST[%d] %s\n", i, tests[i].path);
    }

    printf("[benchmark] Scan complete, powering off\n");
