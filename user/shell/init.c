...
/* ============================================================
 * benchmark mode — auto-test runner
 *
 * Scans /test (reference EXT4 disk) for *_testcode.sh scripts,
...
int main(int argc, char *argv[])
{
    ...
    printf("[benchmark] Auto-test runner started\n");

    chdir(TEST_DIR);

    DIR *d = opendir(TEST_DIR);
    if (!d) {
        printf("[benchmark] Cannot open %s, shutting down\n", TEST_DIR);
    ...
    printf("[benchmark] Found %d test(s)\n", ntests);
    ...
    printf("[benchmark] All tests done, powering off\n");
    syscall(SYS_reboot, 0);
