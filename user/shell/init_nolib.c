/* benchmark init (no-libc): recursive scan + test script listing + poweroff */
...
{
    put_str("[benchmark] Scan-only init started\n");
    put_str("[benchmark] Scanning target image at ");
    put_str(TEST_DIR);
    put_nl();

    scan_tree(TEST_DIR, 0);

    put_str("[benchmark] Found ");
    ...
    put_str("[benchmark] Scan complete, powering off\n");
