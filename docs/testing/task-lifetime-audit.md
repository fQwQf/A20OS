...
| Runqueue | `proc_runq_enqueue_locked()` calls `proc_get()` before setting `on_rq` | `proc_runq_pick_locked()` transfers the same reference to `dispatching`; `proc_runq_remove_locked()` releases it |
...
runs both formal functional tests evaluations.
