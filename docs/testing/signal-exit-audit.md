...
`make check-signal-exit-boundary` enforces the wake-mode markers, signal-lockownership, removal of `proc_make_ready()` fallbacks, interrupt-reason handling,and the targeted test markers.  `make check-proc-step5-local` adds thedual-architecture debug/release stress matrix; `make check-proc-step5`additionally runs both formal functional tests entries.
