...
- **What it checks**: 重新生成 Linux syscall 覆盖表；检查 `LINUX_ABI_BOUNDARY_CONTRACT`、`LINUX_ABI_PLACEHOLDER_RESOLUTION_CONTRACT`、`NATIVE_HANDLE_CAPABILITY_CONSISTENCY_MATRIX`、`NATIVE_DEBUG_LIMITED_CONTRACT` 等静态契约；确认 `docs/native-abi/00-overview.md` 仍包含 `Debug 分区受限`。
...
### Proc/Sched 累计矩阵
- **How to run**: `make check-proc-step8-local`；包含正式双架构 functional tests 时运行 `make check-proc-step8`。
