...
> **平台运行快照**：本页保留 VirtualBox x86_64 配置和逐设备验收方法。审计基线 `e33c3219` 中驱动源码与镜像目标仍存在，但本页没有该基线的新 VirtualBox 实机日志；“成功路径”均是应收集的证据或历史观察，不是后续 HEAD 自动通过声明。
...
| 网络 | VirtIO network | `kernel/drivers/net/virtio_net.c`、`virtio-net.a20drv` | 设备类型/transport 必须与当前 ID 表匹配 |
| 输入 | PS/2 键盘鼠标 | `drvmod` 模块 `ps2.a20drv` | x86 板级控制器服务 |
| 输入 | 可选 VirtIO input | `kernel/drvmod/examples/vinput.c` + `kernel/drivers/input/input_mux.c` | 通过旧聚合节点 `/dev/event0` 暴露 |
...
| 有 PCI 行但未绑定 | ID/subsystem、模块描述符与 bus match、模块注册或 probe 返回错误 |
