...
这里汇集了 A20OS 的设计与开发文档。A20OS 是一个**混合内核**：性能关键路径（调度、MM、VFS、页缓存）留在内核态，服务监管与低速设备驱动以可崩溃、可重启的用户态服务运行；Linux ABI（223 个 syscall）兼容现有 musl 生态，Native ABI（109 个 syscall）探索面向能力、句柄与事件的新接口。混合内核的设计参考见 [hybrid-kernel/00-design.md](hybrid-kernel/00-design.md)。
