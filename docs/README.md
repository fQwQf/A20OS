...
- `kernel/abi/linux/syscall_coverage.md`：362 个 syscall 的逐项兼容等级（全部登记、保守 `partial`，含四主线架构编号覆盖说明）
