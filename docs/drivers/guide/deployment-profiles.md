...
输出同时给出该 profile 的模块数和 embedded 静态链接的设备驱动数。非 MCU 架构的 embedded 构建目录带有 `-embedded` 后缀，避免与默认 generic 对象复用；generic 与 STM32 保持既有目录名，兼容现有运行、烧录与测试入口。
