...
> 本文按 A20 原则（[02-native-api-design.md](../research/02-native-api-design.md)、 [04-theory-deep-dive.md](../research/04-theory-deep-dive.md)、 [06-security.md](06-security.md)）对 Native ABI 进行增添与深化： 盘点内核层为 Linux ABI 实现但尚未被 Native 包装的机制，按原则三分 （应包装 / 应拒绝 / 已有等价物），并为"应包装"项设计干净的 Native 接口， 同时收口现有接口的壳/受限语义。Syscall 编号沿用 [03-handle.md §6](03-handle.md) 的分区约定（0x0000-0x0FFF 稳定区）；结构体演进遵循 [01-types.md §2](01-types.md) 的 E-APPEND / E-DEPRECATE / E-RESERVED 规则。
>
