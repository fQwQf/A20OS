...
- [ ] 修复 shmring 进程组退出后 poweroff 路径的 pfa 空闲链损坏（"corrupted next link"，`kernel/mm/frame.c`）。
  ...
  - 入口硬化实施后的 HMP 取证更新（同日）：guard 生效后症状仍为 poweroff 后静默空转，但采样定位精确化——vCPU 稳定停在 `__trap_from_kernel` 保存序列内（pc=...c356，六次采样五次一致），scause=0xf、stval=`0xffffffc05cf1d158` 位于直映射区内部（>= PAGE_OFFSET，通过下界检查）。结论：坏 sp 并非随机垃圾，而是指向直映射区中已释放/未映射页的"格式合法"指针——与 pfa 空闲链损坏同源（页归还后元数据/内容被复用破坏）。下一步：审计 shmring 场景中内核栈/VMO 页在进程退出时的归还与复用路径。
