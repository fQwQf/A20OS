...
本文描述当前 `kernel/proc/` 实现，而不是早期设计草案。任务状态、CPU所有权、Park/Wake token、timeout、信号和引用计数必须作为一个整体理解；只观察 `task_t.state` 无法判断任务是否仍在运行队列、已被某个 CPU 选中，或正在旧内核栈上完成切换。
...
状态只能由 `kernel/proc/task.c`、`park.c`、`sched.c`、`signal.c` 和退出/回收路径中的受控入口改变。对象等待者不能直接写 `PROC_BLOCKED` 或`PROC_READY`。
...
出队不能直接释放 runqueue 引用。本地 picker 把该引用转交给 dispatch；`context_switch()` 再把 dispatch 所有权转为 CPU 所有权。旧任务的 `on_cpu`必须跨越底层 `__switch` 保持有效，直到新任务在自己的内核栈上调用`proc_switch_complete()`。
...
每个 CPU 有 cacheline 分离的 `need_resched`。远程 wake 或更高优先级任务入队时：
...
唤醒原因显式区分 event、timeout、timeout capacity、普通信号、致命信号、task exit、对象关闭和 cancel。syscall 根据原因映射 `EINTR`、`EAGAIN`、timeout 或正常完成，不能把所有 wake 都当作事件成功。
...
`CONFIG_WAIT_TIMER_HEAP_MAX` 是实际容量边界，不允许为了测试静默扩容。

## 8. 信号、STOPPED 与远程退出

共享 `signal_state` 保护 action、process pending、siginfo；同一对象下各 task的 blocked mask、thread pending、`sigsuspend` 和 `sigtimedwait` 交接也受这把锁保护。需要同时持锁时顺序为：
...
PID lookup 必须使用 `proc_find_get()`，并以 `proc_put()` 配对。以下异步结构在保存 task 指针期间各持有一个引用：
...
回收顺序要求先从所有可发现结构中摘除，再释放相应引用。最后一个引用只能在任务已经 unlisted、unhashed、off-rq、非 dispatch/current、无 wait/wake/timeout owner 时销毁对象。

## 10. exec 参数的用户指针边界

`proc_exec()` 位于 Linux syscall ABI 边界，`argv`/`envp` 始终保留用户指针来源。不能用“地址是否落在内核物理映射范围”推断来源；LoongArch 的identity mapping 会与动态 glibc 程序的低用户虚拟地址重叠。参数数组和字符串分别通过 `copy_from_user()`、`user_strncpy()` 复制一次，再构造新地址空间。
...
`check-proc-step8-local` 定义双架构 debug 单核/release 八核累计压力矩阵；`check-proc-step8` 进一步调用双架构正式 CAgent。目标定义不等于 HEAD 已通过；当前结果必须查看与同一提交匹配的日志。单项契约和故障定位见 [testing/testing-gates.md](testing/testing-gates.md)，`testing/*-audit.md` 是历史阶段证明快照。
...
- [EEVDF 调度器设计](eevdf-scheduler.md) 描述**策略**：普通任务如何被选择（加权 vruntime、系统虚拟时间资格门控及 fallback、虚拟截止时间）、slice 旋钮、SMP 空闲窃取。它回答“CPU 空闲时该跑谁”。
