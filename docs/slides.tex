...
\begin{frame}{跨平台硬件矩阵：一套代码，八种指令集}
    \begin{columns}[t]
        \begin{column}{0.5\textwidth}
            \begin{block}{QEMU 虚拟平台（8 个 ISA）}
                \begin{itemize}
                    \item \high{RISC-V 64} / \high{LoongArch 64}（决赛双架构）
                    \item ARM64 / x86\_64 / ARM32 / RISC-V 32 / PPC64LE
                    \item ARMv7-M（STM32 模拟，NOMMU）
                    \item riscv64 / aarch64 / loongarch64 / x86\_64 SMP 已验证
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.47\textwidth}
            \begin{block}{真实硬件与虚拟机}
                \begin{itemize}
                    \item 星光 2（StarFive VisionFive 2，RISC-V）
                    \item 龙芯 LS2K1000（LoongArch）
                    \item 普中玄武 STM32F103ZET6（64\,KiB SRAM MCU）
                    \item \high{VirtualBox}：x86\_64 / aarch64，UEFI + ACPI 引导
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
    \vspace{0.3em}
    \begin{a20keybox}
        \small NOMMU 支持：riscv64 / riscv32 / aarch64 / arm32 / armv7m 五架构可关 MMU 构建，由验证矩阵 \code{smoke-arch-mmu-matrix} 把关。决赛提交 \code{make all} 直接产出 \code{kernel-rv} / \code{kernel-la} / \code{disk.img} / \code{disk-la.img}。
    \end{a20keybox}
\end{frame}
        ...
        \begin{column}{0.42\textwidth}
            \begin{tikzpicture}[baseline={(current bounding box.north)}]
                \begin{axis}[
                    ybar, bar width=26pt,
                    width=0.94\linewidth, height=3.9cm,
                    symbolic x coords={Linux ABI, Native ABI},
                    xtick={Linux ABI, Native ABI}, ymin=0, ymax=470,
        ...
        \begin{column}{0.56\textwidth}
            \begin{table}
                ...
                \begin{tabular}{ll}
                    ...
                    \textbf{测试组} & \textbf{覆盖} \\
                    \midrule
                    \code{smoke-abi-linux} & syscall 语义逐组验证 \\
                    \code{smoke-syscall-ext} & keyring/AIO/io\_uring 等 \\
                    \code{smoke-*-stress} & 8 核并发压力 × 5 组 \\
                    \code{smoke-network-suite} & TCP/UDP/ICMP/DNS \\
                    \code{smoke-qemu-gui-*} & 5 架构桌面行为判帧 \\
                    \code{smoke-native-*}（16 项） & ipc/svc/registry/ubd… \\
                    \code{check-build-matrix-all} & 七架构构建矩阵 \\
                    \bottomrule
                \end{tabular}
            \end{table}
        \end{column}
