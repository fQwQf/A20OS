...
% 让主题与 figures 无论从 docs/ 还是仓库根目录编译都能找到
\makeatletter
\def\input@path{{slides/}{docs/slides/}}
\makeatother
\graphicspath{{slides/figures/}{docs/slides/figures/}{figures/}}
...
\begin{frame}{A20OS 是什么？—— 一页看完成度}
    \begin{columns}[t]
        \begin{column}{0.62\textwidth}
            ...
            \vspace{0.2em}
            \begin{a20keybox}
                \footnotesize 同一内核：从 64\,KiB Flash 的 MCU，跑到QEMU桌面。
            \end{a20keybox}
        \end{column}
...
\begin{frame}{从初赛到决赛：能力跃迁}
    \begin{table}
        ...
        \begin{tabular}{lll}
            \toprule
            \rowcolor{a20navy}\thd{维度} & \thd{初赛} & \thd{决赛（当前）} \\
            \midrule
            支持架构 & 4 个 & \high{7 hosted + ARMv7-M MCU} \\
            \rowcolor{a20light} Linux ABI & 部分 syscall & \high{361 全表覆盖}，含 io\_uring / landlock / mseal \\
            Native ABI & 90 syscall & \high{139 syscall} + Pager / monitor 深化 \\
            \rowcolor{a20light} 调度器 & 8 级 MLFQ & \high{EEVDF} + per-CPU 队列 + 空闲窃取 \\
            桌面 & -- & \high{Wayland 桌面 + virtio-gpu 3D} \\
            \rowcolor{a20light} 混合内核服务 & -- & \high{svcmgr / 用户态驱动 / KEP 扩展} \\
    ...
    \vspace{0.2em}
    \begin{center}
        \small 从“能启动、能跑几个程序”进化到\high{可跑完整桌面、可跑决赛高性能负载}的通用内核。
    \end{center}
\end{frame}
...
\begin{frame}{从零开始：不基于任何现有内核}
    \begin{columns}[t]
        ...
        \begin{column}{0.41\textwidth}
            ...
            \vspace{-0.4em}
            \begin{center}
                \tiny \tikz[baseline=-0.55ex]{\fill[a20blue] (0,0) rectangle (0.75ex,0.75ex);} 自研内核 15.7 万 ~\tikz[baseline=-0.55ex]{\fill[a20accent] (0,0) rectangle (0.75ex,0.75ex);} 自研用户态 3.5 万 ~\tikz[baseline=-0.55ex]{\fill[a20gray!45] (0,0) rectangle (0.75ex,0.75ex);} lwIP（隔离）6.5 万
            \end{center}
            \vspace{0.3em}
            \begin{a20keybox}
        ...
        \begin{column}{0.63\textwidth}
            \begin{tikzpicture}[baseline={(current bounding box.north)}]
                \begin{axis}[
                    ...
                    ylabel={内核代码累计（千行）/ 周提交次数}, ylabel style={font=\scriptsize},
                    xtick={1,5,10,15,20},
                ...
                \end{axis}
            \end{tikzpicture}
            \srcnote{\textcolor{a20blue}{——} 内核自研累计（千行）~\tikz[baseline=-0.55ex]{\fill[a20accent!40] (0,0) rectangle (0.75ex,0.75ex);} 周提交次数~\tikz[baseline=-0.45ex]{\fill[a20amber] (0,0) circle (0.42ex);} 里程碑。两者数值同量级故共轴；口径：git numstat 周净增（排除 \code{external/}），曲线单调、无跳变导入。}
        \end{column}
    ...
    \begin{columns}[t]
        \begin{column}{0.58\textwidth}
            \begin{table}
                ...
                \begin{tabular}{lcccc}
                    \toprule
                    \rowcolor{a20navy}\thd{架构} & \thd{SMP} & \thd{NOMMU} & \thd{GUI} & \thd{真机 / VM} \\
                    \midrule
                    \high{RISC-V 64}$\star$ & \good{\checkmark} & \good{\checkmark} & \good{\checkmark} & \good{\checkmark} 星光 2 \\
                    \rowcolor{a20light} \high{LoongArch 64}$\star$ & \good{\checkmark} & -- & \good{\checkmark} & \good{\checkmark} LS2K1000 \\
                    aarch64 & \good{\checkmark} & \good{\checkmark} & \good{\checkmark} & \good{\checkmark} VirtualBox \\
                    \rowcolor{a20light} x86\_64 & \good{\checkmark} & -- & \good{\checkmark} & \good{\checkmark} VirtualBox \\
                    arm32 & -- & \good{\checkmark} & \good{\checkmark} & -- \\
                    \rowcolor{a20light} riscv32 & -- & \good{\checkmark} & -- & -- \\
                    ppc64le & -- & -- & -- & -- \\
                    \rowcolor{a20light} armv7m（MCU） & -- & \good{\checkmark}（仅） & -- & \good{\checkmark} STM32 \\
                    \bottomrule
                \end{tabular}
            \end{table}
        \end{column}
        ...
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{Linux ABI 兼容层 \code{kernel/abi/linux/}}
                \begin{itemize}
                    ...
                    \item 直接运行 musl 生态与完整 Linux 用户态
                \end{itemize}
            \end{alertblock}
        \end{column}
        \begin{column}{0.48\textwidth}
...
\begin{frame}{动态链接与用户态生态}
    \begin{columns}[t]
        ...
        \begin{column}{0.42\textwidth}
            ...
            \vspace{0.3em}
            \begin{a20warnbox}
                \footnotesize 这是完整桌面能起来的地基；早期 musl 加载期空指针崩溃即从 exec/mmap 语义修正而来。
            \end{a20warnbox}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[t]
        ...
        \begin{column}{0.42\textwidth}
            \begin{tikzpicture}[baseline={(current bounding box.north)},
                ...
                % 网格与基线（361 -> 3.3cm）
                \foreach \gy in {0,100,200,300} {
                    \draw[a20navy!10] (0.2,\gy*0.00914) -- (5.3,\gy*0.00914);
                    \node[left, font=\tiny, text=a20gray] at (0.2,\gy*0.00914) {\gy};}
                \draw[a20navy!35] (0.2,0) -- (5.3,0);
                % 三根柱：Linux ABI 361 / Native ABI 139 / xv6 23
                \fill[a20blue]   (0.55,0) rectangle (1.45,3.298);
                \fill[a20accent] (2.35,0) rectangle (3.25,1.270);
                \fill[a20gray!45] (4.15,0) rectangle (5.05,0.210);
                \node[above, font=\scriptsize\bfseries, text=a20navy] at (1.0,3.298) {361};
                \node[above, font=\scriptsize\bfseries, text=a20navy] at (2.8,1.270) {139};
                \node[above, font=\scriptsize\bfseries, text=a20navy] at (4.6,0.210) {23};
                \node[below, text=a20ink] at (1.0,0) {Linux ABI};
                \node[below, text=a20ink] at (2.8,0) {Native ABI};
                \node[below, text=a20ink] at (4.6,0) {xv6 参照};
            \end{tikzpicture}\par
    ...
    \begin{columns}[t]
        \begin{column}{0.52\textwidth}
            ...
            \begin{itemize}
                ...
                \item 帧调度链完整：commit $\rightarrow$ render $\rightarrow$ pageflip $\rightarrow$ vblank
                \item \high{LVGL 自研桌面}：仪表盘 / 终端 / 文件 / 进程监控
            \end{itemize}
            \vspace{0.2em}
            \srcnote{关键文件：\code{kernel/drivers/gpu/drm.c}；验证 \code{make smoke-qemu-gui-riscv64}（QMP 注入键鼠 + 截图判帧）}
        \end{column}
        \begin{column}{0.45\textwidth}
            ...
            \end{tcolorbox}
            \vspace{-0.3em}
            \srcnote{实拍：riscv64 QEMU，Weston 桌面内播放视频。}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{决赛 BuildStorm：双架构 180/180，耗时下降约 40\%}
    \begin{columns}[t]
        \begin{column}{0.56\textwidth}
            ...
            \vspace{-0.4em}
            \begin{center}
                \tiny \tikz[baseline=-0.55ex]{\fill[a20blue] (0,0) rectangle (0.75ex,0.75ex);} RISC-V64 ~~ \tikz[baseline=-0.55ex]{\fill[a20accent] (0,0) rectangle (0.75ex,0.75ex);} LoongArch64
            \end{center}
            \srcnote{快照 d5ae5a16 / f3e3ce48 / f9732348，同一 workload、8 vCPU、TCG multi；与 Linux 基线跨 QEMU 版本。}
        \end{column}
        \begin{column}{0.41\textwidth}
    ...
    \begin{columns}[t]
        \begin{column}{0.52\textwidth}
            \begin{block}{STM32F103（ARMv7-M / NOMMU）}
                ...
                \end{itemize}
            \end{block}
            \begin{block}{PPC64LE 与跨平台工程}
                \footnotesize QEMU pSeries 固件（MMU 单核边界）；统一 arch/HAL 抽象 + 七架构构建矩阵，物理板与虚拟平台同构验证。
            \end{block}
            \vspace{0.1em}
            \srcnote{入口：\code{make stm32f103-xuanwu}、\code{make run-stm32f103-qemu}、\code{make ARCH=ppc64le run}}
        \end{column}
        \begin{column}{0.45\textwidth}
            \begin{tcolorbox}[colback=a20navy, colframe=a20navy, arc=1mm, boxrule=0pt, left=2pt, right=2pt, top=2pt, bottom=2pt]
                \includegraphics[width=\linewidth, trim=0 190 0 0, clip]{stm32.jpg}
            \end{tcolorbox}
            \vspace{-0.3em}
            \srcnote{实拍：普中玄武 STM32F103ZET6 运行 A20OS，LCD 仪表盘 + 舵机 / DHT11 / 蓝牙外设。}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{测试门禁体系：54 项冒烟全部可复现}
    \begin{columns}[t]
        \begin{column}{0.55\textwidth}
            \begin{tikzpicture}[baseline={(current bounding box.north)}]
                \begin{axis}[
                    a20bar, width=0.88\linewidth, height=5.6cm,
    ...
    \begin{columns}[t]
        ...
        \begin{column}{0.42\textwidth}
            \begin{a20keybox}
                ...
                \vskip0.2em
                \code{make smoke-abi-linux} 逐组语义验证；\code{smoke-syscall-ext} 覆盖 io\_uring / landlock / userfaultfd / perf 等现代机制；真实负载：musl 生态 + Cargo 全量编译。
            \end{a20keybox}
        \end{column}
    \end{columns}
\end{frame}
