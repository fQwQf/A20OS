...
> **历史计划说明（2026-07）**：本文记录从 53-syscall 设计走向首个实现的原始方案，文件路径、分派伪代码、阶段状态和 LOC 估算不再代表当前仓库。当前实现为 126 个 syscall，按 `task->abi_mode` 分派；typed channel、时态控制/sweeper、阻塞 IPC、对象级联释放和 VMO source 映射状态见 `docs/native-abi/08-runtime-status.md`。本文伪代码不得作为现行并发或 ABI 契约。
