...
> **本文是预算能力（03）的"传播维度"**：在预算格（类型 × 权限 × 时间 × 次数 × 传播）中，typed channel 实现的是**传播维度**——能力能流向谁、流什么类型。它与时间/次数维度（03）组合构成供应链防御（03 定理 7.1），并支撑 05 信封的"可向外传播的类型"策略。在论文中作为 C1 的组成部分与信封的支撑材料，不单独主张贡献。
>
> 基础 SOS 模型见 06。
