...
>
> **范围说明：** 本文是实验设计快照，不是 `e33c3219` 的实测报告。阈值、预期延迟、样本量和命令均是待验证方案；未填结果不得解释为当前性能结论。现有正式性能样本及其可比性边界见一手审计文档 [A20OS 与 Linux 实现及性能差异](10-a20-linux-implementation-performance-comparison.md)；该文列出的 `.kernel-build/smoke/2026/` 路径是被 Git 忽略的本地工作区证据归档，干净 clone 不包含这些文件。
