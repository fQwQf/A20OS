...
| **类型化控制（替代 ioctl）** | 已实现（Native ABI 无 ioctl） | `handle_control` 版本化 op（WINSIZE/TCFLUSH/SET_FLAGS），03-handle §2.7 |
...
- `linux-audit-and-performance-comparison.md`：A20 vs Linux 的源码审计与 BuildStorm 实测边界（差距约 2.3-2.5×，工程侧）。
