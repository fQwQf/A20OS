# A20OS BuildStorm 热路径优化：分桶锁与 fsync 收敛（`fqwqf/performance-overhaul`）

> 分支：`fqwqf/performance-overhaul`，基自 `main` (`ca6c7008`)。
> 目标：把 8 核 BuildStorm 中"page cache / block cache / dcache 单全局锁 + fsync 全盘写回"的
...
与本节改动无关。审计基线的完整 BuildStorm 重跑仍按 `docs/BuildStorm-2026.md` 的边界执行。
