...
| `make final-all` / `make all-architectures` | `make all` 的显式别名 | 提交前完整编译 |
...
-  `make all` 内部固定使用 `PROFILE=benchmark NR_CPUS=8 DRIVER_DEPLOYMENT=embedded`，不受默认单核开发配置影响，并兼容测试命令默认的 legacy VirtIO-MMIO 块设备；parallel-build 设计与复现说明见 [parallel-build-2026.md](parallel-build-2026.md)。
