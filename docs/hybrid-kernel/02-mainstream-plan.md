...
**当前状态**：`f9732348` 曾完成 RISC-V64/LoongArch64 整体决赛平台流程，但这不覆盖所有 Native/hybrid smoke，也不能外推到 `e33c3219`。当前多个 Native、mlibc、dual-placement smoke 仍硬编码 RISC-V64；loongarch64 vDSO 仍未实现。性能数据全部来自 QEMU TCG。
...
- **页缓存与文件系统留内核**：udisk 形态是"页缓存留内核 + 块请求进用户驱动"，与主流存储栈一致；历史 TCG 样本显示页缓存命中路径吸收了用户驱动开销，本次未重测。
