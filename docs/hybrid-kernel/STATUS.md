...
本文档以清单形式说明 A20OS 混合内核**当前源码具备的能力**与**已知边界**。源码已按 `e33c3219` 核对；当前提交没有匹配的完整干净双架构平台运行。最新完整平台证据是 `f9732348` 的双架构决赛流程，但它既早于当前提交，也不覆盖全部 Native/hybrid smoke。设计总览见 [00-design.md](00-design.md)，机制语义见 [01-mechanisms.md](01-mechanisms.md)，演进路线见 [03-refactor-plan.md](03-refactor-plan.md)。
...
- **loongarch64**：`f9732348` 有整体决赛平台成功证据，但当前多个 Native/dual/mlibc smoke 仍只运行 RISC-V64，且 `e33c3219` 无匹配完整复验；
