...
A20OS 同时提供两套用户接口：`abi/linux`（223 个 syscall，运行 musl 程序，是当前主用户态运行时）与 `abi/native`（112 个 syscall，面向能力、句柄与事件的新接口）。混合内核是这两套接口共享的执行底座：**把性能关键路径留在内核态，把可崩溃、可重启的用户态服务作为系统组成部分**。
