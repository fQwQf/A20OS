...
int64_t sys_chroot(const char *path)
{
    ...
    if (!cur || cur->euid != 0) return -EPERM;
    strncpy(cur->root_path, full, MAX_PATH_LEN - 1);
    cur->root_path[MAX_PATH_LEN - 1] = '\0';
    size_t len = strlen(cur->root_path);
    while (len > 1 && cur->root_path[len - 1] == '/')
        cur->root_path[--len] = '\0';
    cur->cwd[0] = '/';
    cur->cwd[1] = '\0';
    return 0;
