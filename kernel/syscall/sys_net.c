...
int64_t sys_socket(int domain, int type, int protocol) {
    ...
    return syscall_alloc_local_fd_with_flags(t, gfd, type);
}
...
int64_t sys_socketpair(int domain, int type, int protocol, int *sv) {
    ...
    if (r < 0) return r;
    task_t *t = proc_current();
    int l0 = syscall_alloc_local_fd(t, gfds[0]);
    if (l0 < 0) {
        vfs_close(gfds[1]);
        return l0;
    }
    int l1 = syscall_alloc_local_fd(t, gfds[1]);
    if (l1 < 0) {
        vfs_close(t->fd_table[l0]);
        t->fd_table[l0] = -1;
        t->fd_cloexec[l0] = 0;
        return l1;
    }
...
int64_t sys_sendto(int fd, const void *buf, size_t len, int flags,
    ...
    if (len && copy_from_user(kbuf, buf, len) < 0) { kfree(kbuf); return -EFAULT; }
    uint8_t kaddr[NET_SOCKADDR_MAX];
    const void *ka = NULL;
    if (addr) {
        int r = copy_sockaddr_from_user(kaddr, addr, addrlen);
