...
static int memfd_install_vfile(vfile_t *vf, int flags)
{
...
static int64_t memfd_create_file(int flags)
{
    ...
    return memfd_install_vfile(vf, flags);
}
...
int64_t sys_shmat(int shmid, const void *shmaddr, int shmflg)
{
    ...
    return sys_mmap((uint64_t)shmaddr, g_shm[shmid].size, PROT_READ | PROT_WRITE,
