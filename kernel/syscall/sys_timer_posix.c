...
int64_t sys_setitimer(int which, const void *new_value, void *old_value)
{
    ...
    if (which == 0) {
        ...
        cur->alarm_expire = ticks ? timer_get_ticks() + ticks : 0;
    }
    return 0;
}
...
int64_t sys_alarm(unsigned seconds)
{
    ...
    cur->alarm_expire = seconds ? now + (uint64_t)seconds * TICKS_PER_SEC : 0;
    return old;
}
...
int64_t sys_adjtimex(void *buf)
{
    ...
    if (modes != ADJ_OFFSET_SINGLESHOT && modes != ADJ_OFFSET_SS_READ &&
        ...
        return -EINVAL;
    task_t *cur = proc_current();
    if (modes && modes != ADJ_OFFSET_SS_READ && cur && cur->euid != 0)
        return -EPERM;
