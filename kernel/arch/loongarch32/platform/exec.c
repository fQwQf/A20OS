...
int arch_resolve_interp_fallback(const char *exec_path, const char *interp_path,
    ...
    if (interp_path && strstr(interp_path, "ld-musl-loongarch")) {
        if (path_build_sibling(exec_path, "/lib/libc.so",
                               resolved, resolved_size) == 0)
            return 0;
        if (path_build_sibling(exec_path, "/lib32/libc.so",
                               resolved, resolved_size) == 0)
            return 0;
    }

    if (interp_path && strstr(interp_path, "ld-linux-loongarch")) {
        static const char *glibc_paths[] = {
            "/lib/ld-linux-loongarch-lp32d.so.1",
            "/lib/ld-linux-loongarch32.so.1",
        };
