...
static uintptr_t alloc_bar_mem(uint8_t dev, int bar) {
    ...
    uintptr_t addr = (pci_mmio_alloc + size - 1) & ~(uintptr_t)(size - 1);
    pci_mmio_alloc = addr + size;

    ecam_write(0, dev, 0, PCI_BAR0 + bar * 4, (uint32_t)(addr - PAGE_OFFSET));
    return addr;
}
...
static int find_virtio_caps(int dev, pci_virtio_dev_t *vd) {
    ...
    while (ptr && ptr < 0xFF) {
        ...
        uintptr_t bar_base = arch_pci_bar_to_resource(bar_val & ~0xFUL);
...
void pci_init(void) {
    ...
    pci_inited = 1;
    arch_pci_host_init(PCI_ECAM_BASE);
...
static uint32_t pci_vt_read32(virtio_transport_t *t, uint32_t mmio_off) {
    ...
    case VIRTIO_MMIO_INTERRUPT_STATUS:
        if (vd->isr_base)
            return *(volatile uint8_t *)(vd->isr_base);
        return 0;
