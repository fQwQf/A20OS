...
static inline void arch_task_context_set_initial_sp(task_context_t *ctx,
    ...
    (void)trap;
    (void)stack_top;
    /* LoongArch __switch computes the resume stack pointer from the context
     * base, so the saved sp field is ignored. */
}
...
static inline task_context_t *arch_task_context_base(void *kstack_base,
                                                      ...
                                                      trap_context_t *trap) {
    (void)kstack_base;
    /* LoongArch __switch resumes at ctx + 120.  User tasks resume on the
