#ifndef _ARCH_RISCV32_SIGNAL_ABI_H
#define _ARCH_RISCV32_SIGNAL_ABI_H
...
static inline void arch_ucontext_sigmask_set(arch_ucontext_t *uc, uint64_t mask) { uc->uc_sigmask.bits[0] = (uint32_t)(mask >> 1); uc->uc_sigmask.bits[1] = (uint32_t)((mask >> 33) & 0x7fffffffU); }
...
static inline arch_sigset_t arch_user_sigset_from_kernel(uint64_t mask) { arch_sigset_t set = { .bits = { (uint32_t)(mask >> 1), (uint32_t)((mask >> 33) & 0x7fffffffU) } }; return set; }
static inline uint64_t arch_user_sigset_to_kernel(const arch_sigset_t *mask) { return ((uint64_t)mask->bits[0] << 1) | ((uint64_t)(mask->bits[1] & 0x7fffffffU) << 33); }
...
#endif /* _ARCH_RISCV32_SIGNAL_ABI_H */
