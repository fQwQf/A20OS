...
static int build_sibling_path(const char *exec_path, const char *suffix, char *resolved, size_t resolved_size) {
...
static int build_mount_root_path(const char *exec_path, const char *suffix, char *resolved, size_t resolved_size) {
    if (!exec_path || exec_path[0] != '/' || !suffix || suffix[0] != '/' || !resolved || resolved_size == 0)
        return -ENOENT;
    int end = 1;
...
int arch_resolve_interp_fallback(const char *exec_path, const char *interp_path, char *resolved, size_t resolved_size) {
    if (interp_path && strstr(interp_path, "ld-musl-armhf.so.1") &&
        build_sibling_path(exec_path, "/lib/libc.so", resolved, resolved_size) == 0)
        return 0;
    if (interp_path && strstr(interp_path, "ld-linux-armhf.so.3") &&
        build_mount_root_path(exec_path, "/glibc/lib/ld-linux-armhf.so.3", resolved, resolved_size) == 0)
        return 0;
    return -ENOENT;
}
