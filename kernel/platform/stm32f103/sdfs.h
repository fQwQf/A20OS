...
/*
 * TF-card filesystem: mounts the FAT32 layer (fat32.c) on top of the SDIO
...
/* Mount the card's FAT32 volume. Returns FAT32_OK, or a negative FAT32_E* /
 * FAT32_EIO when the card is absent or unreadable. Safe to call again. */
int stm32_sdfs_mount(void);

int stm32_sdfs_ready(void);

/* The mounted filesystem, or NULL if not mounted. */
fat32_fs_t *stm32_sdfs(void);

/* Convenience whole-file helpers (mount first). read returns bytes read or a
 * negative FAT32_E*; write creates/truncates then writes `len` bytes. */
 ...
 * or a negative FAT32_E* when the file is missing/unreadable. */
int stm32_sdfs_load_config(hub_cfg_t *out);

/* Append a timestamped line to /LOG/RUN.LOG (creating /LOG on first use).
 * ts_ms is uptime in ms. Returns FAT32_OK or a negative FAT32_E*. */
